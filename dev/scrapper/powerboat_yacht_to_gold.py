...
def parse_main_entity_fixed(text):
    ...
    needs_headers = {'Deadweight, t', 'Fuel capacity, l', 'Water tank, l', 'Max people', 'Cabins', 'Berths for guests',
                     ...
                     'Displacement, t', 'Crew', 'Bathrooms', 'Model', 'Aim of usage', 'transom deadrise angle, degrees'}
    ...
    while i < len(lines) - 1:
        if lines[i].lower().strip() not in needs_headers_lower:
            ...
            continue
        key = lines[i]
        if key == 'Engine options':
            i += 1
            engines = []
            while lines[i].lower().strip() not in needs_headers_lower:
                engines.append(clean_value(lines[i]))
                i += 1
            result['engines'] = '\n'.join(engines)
        else:
            val = lines[i + 1]
...
if __name__ == '__main__':
    FILE_NAME = "data/boats_itboat_PB.csv"
    GOLD_FILE_NAME = "data/boats_itboat_PB_gold.csv"
